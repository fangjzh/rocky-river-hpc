...
# 同步 CRB 仓库
sync_crb_repo() {
    local step="sync_crb_repo"
    if step_done "$step"; then
        log "CRB repo already synced. Skipping."
        return 0
    fi

    log "Syncing CRB repository..."
    mkdir -p /opt/repo/rocky
    reposync --repoid=kickstart-crb --exclude 'java-*debug*' --exclude 'dotnet*' -p /opt/repo/rocky/
    check_success "Failed to sync CRB repo"
    createrepo /opt/repo/rocky/kickstart-crb
    cd /opt/repo/rocky && tar -cf /root/kickstart-crb.tar kickstart-crb
    mark_done "$step"
}
...
configure_extra_repos() {
    ...
    safe_wget "/etc/yum.repos.d/fish.repo" "https://download.opensuse.org/repositories/shells:/fish:/release:/3/CentOS-9_Stream/shells:fish:release:3.repo"
    mark_done "$step"
}
...
download_openhpc() {
    ...
    # 删除 trilinos 包，瘦身
    rm -f /opt/repo/openhpc/EL_9/x86_64/trilinos-*
    rm -f /opt/repo/openhpc/EL_9/updates/x86_64/trilinos-*
    rm -f /opt/repo/openhpc/EL_9/x86_64/*parallel-libs*
    rm -f /opt/repo/openhpc/EL_9/updates/x86_64/*parallel-libs*

    # 删除 intel 相关包，它依赖Intel OneAPI, 目前没有打算去集成Intel OneAPI 的repo，而是后续采用离线方式安装
    rm -f /opt/repo/openhpc/EL_9/x86_64/*intel*
    rm -f /opt/repo/openhpc/EL_9/updates/x86_64/*intel*  

    createrepo /opt/repo/openhpc/EL_9/updates/
    createrepo /opt/repo/openhpc/EL_9/
...
download_openhpc_update() { 
    ...
    local ohpc_url="https://repos.openhpc.community/OpenHPC/3/"
    # @ 2025年7月25日
...
download_xcat() {
    ...
    cd /opt/repo/xcat/xcat-dep && rm -rf rh7 rh8 rh9/ppc64le/ sles12/ sles15/
    /opt/repo/xcat/xcat-dep/rh9/x86_64/mklocalrepo.sh
...
restore_yum_repos() {
    ...
    perl -pi -e "s/enabled=0/enabled=1/" /etc/yum.repos.d/rocky.repo
